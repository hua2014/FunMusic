...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
  ...
  for x in ${dataset_name}_dev ${dataset_name}_train; do
    echo "$x"
    python tools/extract_acoustic_token.py --dir data/$x \
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
  ...
  for x in ${dataset_name}_dev ${dataset_name}_train; do
    echo "$x"
    python tools/extract_semantic_token.py --dir data/$x \
...
if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
  echo "Extract video emb, you should have prepared VidMuse Source, mp4.scp"
  for x in ${dataset_name}_dev ${dataset_name}_train; do
    echo "$x"
    python tools/extract_video_emb.py --dir data/$x
  done
fi
